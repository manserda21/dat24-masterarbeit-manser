...
def detect_camera_segments(homographies: dict,
                            max_center_shift_px: float = 80.0) -> list:
    """
    ...
    max_center_shift_px: maximale erlaubte Verschiebung in Template-Pixeln
                         zwischen zwei aufeinanderfolgenden Keyframes.
    """
    indices = sorted(homographies.keys())

    # Fester Referenzpunkt: Mitte des Bildes (bekannte Bildpixel-Koordinate)
    img_w_ref = 1920.0
    img_h_ref = 1080.0
    ...
    for i in range(1, len(indices)):
        idx = indices[i]
        H = homographies[idx]
        curr_pos = cv2.perspectiveTransform(img_center, H)[0][0]
...
def transform_tracking(df: pd.DataFrame, get_H, total_frames: int) -> pd.DataFrame:
    ...
    # Deine Spaltennamen → interne Namen
    COLUMN_MAP = {
        "frame":    "frame_idx",
        "x_center": "x",
        "y_center": "y",
...
def visualize_trajectories(df: pd.DataFrame, track_id: int = None, output_path: str = None):
    ...
    if track_id is not None:
        ...
        if "track_id" in valid.columns:
            cmap = plt.cm.get_cmap("tab20", valid["track_id"].nunique())
            for i, (tid, group) in enumerate(valid.groupby("track_id")):
                ax.plot(group["x_m"], group["y_m"],
                        linewidth=0.8, alpha=0.6, color=cmap(i))
        else:
...
def main():
    ...
    COLUMN_MAP = {"frame": "frame_idx", "x_center": "x", "y_center": "y"}
    df = df.rename(columns=COLUMN_MAP)
